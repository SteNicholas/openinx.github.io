...
![Alt text](/images/plane-sweep-3.png)
...
__算法分析__  这题可以这样思考：对第i个员工,他在矩形内种植的是第s种种子。可以把该员工的种植情况想象成底面矩形在坐标系上且高为的长方体。
这样，题目就转化成求N个底面在坐标系上的长方体的体积并了。而计算体积时，可以用平面依次去截N个长方体。求N个长方体被平面截得的区域面积就等价于多矩形面积并问题了（参考例A.1）。

于是，总体积就等于。其中。

__复杂度分析__ 最坏情况下，每次都截到N个矩形。求矩形面积并的时间复杂度为。共要截m次，所以总的时间复杂度为。[代码参见附录A.2]
